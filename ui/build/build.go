...
func Build(ctx Context, config Config, what int) {
	...
	if totalRAM := config.TotalRAM(); totalRAM != 0 {
		...
		if ram <= 16 {
			...
			ctx.Println("")
			ctx.Println("The source code contains patches")
			ctx.Println("so the minimum required RAM is 8GB.")
			ctx.Println("But be careful because the build may fail")
			ctx.Println("with Out-Of-Memory error.")
			ctx.Println("")
